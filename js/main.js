...
for (let i = 0; i < quickSpan.length; i++) {
  ...
  for (let j = 0; j < 20; j++) {
    if (j < 10) {
      images += `<img src="images/quick0${i + 1}/quick0${i + 1}_0000${j}.png" />`;
    } else {
      images += `<img src="images/quick0${i + 1}/quick0${i + 1}_000${j}.png" />`;
    }
  }
...
let bnnWidth = document.querySelector('body>section').offsetWidth; // section의 width 값 할당
...
for (let i = 0; i < bnnSection.length; i++) {
  sectionImg = `url(images/banner/banner${i}.jpg) no-repeat 50% 0`;
...
// left 값 변경, 클래스 추가/삭제
// 섹션에 white 포함 -> 버튼/롤링에 white 클래스 추가
...
let changeCls = () => {
  if (htmlWidth > 768) {
    classLeft = 6; // tablet, PC 6개
  } else {
    classLeft = 4; // mobile 4개
  }
};
changeCls();
window.addEventListener('resize', () => {
...
// 브랜드 li와 변수 값과 비교 후 switch case문 사용
